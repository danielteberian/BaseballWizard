...
# Calculate away performance of each team
home_team_performance = spark.sql("""
SELECT
	home_team AS team,
	AVG(home_score) AS avg_home_score,
	AVG(home_hits) AS avg_home_hits,
	AVG(home_avg_pitch_speed) AS avg_home_pitch_speed,
	AVG(home_avg_spin_rate) AS avg_home_spin_rate
FROM statcast
GROUP BY home_team
""")
...
# Calculate away performance of each team
away_team_performance = spark.sql("""
SELECT
	away_team AS team,
	AVG(away_score) AS avg_away_score,
	AVG(away_hits) AS avg_away_hits,
	AVG(away_avg_pitch_speed) AS avg_away_pitch_speed,
	AVG(away_avg_spin_rate) AS avg_away_spin_rate)
FROM statcast
GROUP BY away_team
""")
...
game_stats = game_stats \
	.join(home_team_performance, game_stats["game_home_team"] == home_team_performance["team"], "left") \
	.join(away_team_performance, game_stats["game_away_team"] == away_team_performance["team"], "left")
